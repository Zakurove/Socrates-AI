...
import {
  useAdminPatchStationVisibility,
  useAdminStations,
...
function AdminStationsInner() {
  ...
  const [confirmPrivatize, setConfirmPrivatize] =
    useState<AdminStationRow | null>(null);

  const patch = useAdminPatchStationVisibility();

  useEffect(() => {
    document.title = "Stations — Admin";
  }, []);
  ...
  const { data, isLoading, isError, refetch } = useAdminStations({
    visibility,
    q: debounced,
    offset,
    limit: PAGE_SIZE,
    ...
    <div className="min-h-screen bg-background pb-12">
      ...
      <main className="mx-auto max-w-[900px] px-5 pt-6">
        <header className="mb-6">
          <h1 className="font-display text-h1 text-foreground">Stations</h1>
        ...
        {isLoading ? (
          ...
          <div className="rounded-2xl border border-dashed border-destructive/40 bg-destructive/5 px-5 py-6 text-center">
            <p className="text-body text-foreground">Couldn't load stations.</p>
            <Button
              variant="outline"
          ...
          <div className="rounded-2xl border border-dashed border-border/70 bg-card/40 px-5 py-10 text-center">
            <p className="text-body text-muted-foreground">
              {debounced ? "No matching stations." : "No stations."}
            </p>
          </div>
        ) : (
          <Card>
            <CardContent className="divide-y divide-border/60 p-0">
              {rows.map((s) => (
                <StationRow
                  key={s.id}
                  row={s}
                  onOpen={() => navigate(`/station/${s.id}`)}
                  onAuthor={() => navigate(`/admin/users/${s.author.id}`)}
                  onSetVisibility={(v) => setVisibilityFor(s, v)}
      ...
      >
        <AlertDialogContent>
          <AlertDialogHeader>
            <AlertDialogTitle>Unpublish this station?</AlertDialogTitle>
            <AlertDialogDescription>
  ...
  return (
    <div className="flex items-center gap-3 px-4 py-3 transition-colors hover:bg-muted/40">
      ...
      <div className="hidden shrink-0 items-end gap-4 text-right text-[12px] text-muted-foreground sm:flex">
        ...
        <Stat label="Practices" value={row.practiceCount} />
        <Stat
          label="Reports"
      ...
      <DropdownMenu>
        ...
        <DropdownMenuContent align="end">
          <DropdownMenuItem onClick={onOpen}>View station</DropdownMenuItem>
          <DropdownMenuItem onClick={onAuthor}>View author</DropdownMenuItem>
          <DropdownMenuSeparator />
          <DropdownMenuItem
